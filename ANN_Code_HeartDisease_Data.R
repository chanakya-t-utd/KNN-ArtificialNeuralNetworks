...
dataset1 <- read.xlsx("C:/Users/meera.kulkarni.BEST/Desktop/UTD/Machine_Learning/Project2/HeartDisease_Dataset.xlsx", sheet = "Raw", colNames = TRUE, detectDates = TRUE)
for (i in 1:303){
...
set.seed(2)
NN = neuralnet(target~age+sex+trestbps+chol+restecg+slope+fbs+thalach+exang+oldpeak+cp, trainNN,
             hidden = 4 , linear.output = T, act.fct = 'tanh') #Change act.fct to vary differentiating function, hidden to layers
plot(NN)
NN$

predict_testNN = compute(NN, testNN)
predict_testNN = (predict_testNN$net.result * (max(data$target) - min(data$target))) 
                   + min(data$target)
...
CM<-table(predict_testNN,testNN$target) #Confusion Matrix
